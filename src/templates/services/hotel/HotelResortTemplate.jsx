...
export default function HotelResortTemplate({
  ...
  const conciergeContent = getSectionContent(effectiveSections, "reservation_concierge") || {};
  const suitesContent = getSectionContent(effectiveSections, "suites") || {};
  const expContent = getSectionContent(effectiveSections, "experiences") || {};
  ...
  return (
    ...
    >
      ...
      <main className="flex-1">
            ...
            <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 text-center text-white py-20">
              ...
              <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/10 backdrop-blur-md border border-white/20 mb-6">
                ...
                <span className="text-[11px] font-bold uppercase tracking-[0.25em] text-[#F3E5AB]">
                  {heroContent.badge || "Forbes Five-Star Rated Luxury Sanctuary"}
                </span>
              </div>
              ...
              <div className="mt-8 flex flex-wrap items-center justify-center gap-4">
                ...
                >
                  <Calendar size={15} />
                  <span>{heroContent.ctaText || "Reserve Your Suite"}</span>
                </button>
                <a
                  href="#suites"
        ...
        {isSectionEnabled(effectiveSections, "reservation_concierge") && (
          <section className="relative z-20 -mt-10 max-w-6xl mx-auto px-4 sm:px-6">
            <div className="bg-white rounded-3xl shadow-2xl border border-stone-200/80 p-5 sm:p-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
              ...
              <div>
                ...
                >
                  <span>{conciergeContent.ctaText || "Check Rates"}</span>
                  <ArrowRight size={14} />
                </button>
              </div>
            </div>
        ...
        {isSectionEnabled(effectiveSections, "suites") && (
          <section id="suites" className="py-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center max-w-2xl mx-auto mb-12">
              <span className="text-[11px] font-bold uppercase tracking-[0.25em] text-[#9A7B4F] block mb-2">
                {suitesContent.badge || "Bespoke Sanctuary Collection"}
              </span>
              <h2 className="text-3xl sm:text-4xl font-serif font-bold text-stone-900 tracking-tight">
                {suitesContent.title || "Suites, Overwater Villas & Penthouses"}
              </h2>
              <p className="text-xs sm:text-sm text-stone-500 font-light mt-2">
                {suitesContent.subtitle || "Each residence features custom Italian linens, private plunge pools, panoramic glass balconies, and tailored 24-hour butler services."}
              </p>
        ...
        {isSectionEnabled(effectiveSections, "experiences") && (
          <section className="py-20 bg-stone-100/60 border-y border-stone-200/60">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="text-center max-w-xl mx-auto mb-14">
                <span className="text-[11px] font-bold uppercase tracking-[0.25em] text-[#9A7B4F] block mb-2">
                  {expContent.badge || "Unrivaled Hospitality"}
                </span>
                <h2 className="text-3xl font-serif font-bold text-stone-900">
                  {expContent.title || "World-Class Resort Experiences"}
                </h2>
                <p className="text-xs text-stone-500 font-light mt-2">
                  {expContent.subtitle || "Every moment designed to immerse you in tranquility, indulgence, and unforgettable memories."}
                </p>
              </div>
              ...
              <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
                <div className="lg:col-span-5 space-y-4">
                  ...
                  <h3 className="text-2xl sm:text-3xl font-serif font-bold tracking-tight">
                    {revContent.title ? `"${revContent.title}"` : '"The most transcendent coastal sanctuary we have ever experienced."'}
                  </h3>
                  <p className="text-xs text-stone-400 font-light leading-relaxed">
                    {revContent.subtitle || "Celebrated by Condé Nast Traveler and Forbes Guide as one of the world’s top boutique resort destinations."}
                  </p>
                  <div className="flex items-center gap-3 pt-2">
                    <div className="w-10 h-10 rounded-full bg-[#9A7B4F] flex items-center justify-center font-serif font-bold text-white text-sm">
                      {revContent.authorInitials || "EH"}
                    </div>
                    <div>
                      <h5 className="font-bold text-xs text-white">{revContent.authorName || "Eleanor & Harrison Wright"}</h5>
                      <p className="text-[10px] text-stone-400">{revContent.authorLocation || "London, United Kingdom • Ocean Villa Guests"}</p>
                    </div>
                  </div>
                </div>
