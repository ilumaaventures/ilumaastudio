...
export default function TravelAgencyTemplate({
  ...
  const filterContent = getSectionContent(effectiveSections, "search_filter") || {};
  const packagesContent = getSectionContent(effectiveSections, "packages") || {};
  const pillarsContent = getSectionContent(effectiveSections, "pillars") || {};
  const reviewsContent = getSectionContent(effectiveSections, "reviews") || {};
  ...
  return (
    ...
    >
      ...
      <main className="flex-1">
            ...
            <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 text-center text-white py-20">
              ...
              <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-sky-500/20 backdrop-blur-md border border-sky-400/30 mb-6">
                ...
                <span className="text-[11px] font-bold uppercase tracking-[0.25em] text-sky-200">
                  {heroContent.badge || "Award-Winning Luxury Travel Agency & Tour Operator"}
                </span>
              </div>
              ...
              <div className="mt-8 flex flex-wrap items-center justify-center gap-4">
                ...
                >
                  <Plane size={15} />
                  <span>{heroContent.ctaText || "Enquire Tour Package"}</span>
                </button>
                <a
                  href="#packages"
        ...
        {isSectionEnabled(effectiveSections, "search_filter") && (
          <section className="relative z-20 -mt-10 max-w-6xl mx-auto px-4 sm:px-6">
            <div className="bg-white rounded-3xl shadow-2xl border border-sky-100 p-5 sm:p-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
              ...
              <div>
                ...
                >
                  <span>{filterContent.ctaText || "Find Expeditions"}</span>
                  <ArrowRight size={14} />
                </button>
              </div>
            </div>
        ...
        {isSectionEnabled(effectiveSections, "packages") && (
          <section id="packages" className="py-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center max-w-2xl mx-auto mb-12">
              <span className="text-[11px] font-bold uppercase tracking-[0.25em] text-[#0284C7] block mb-2">
                {packagesContent.badge || "Signature Worldwide Catalog"}
              </span>
              <h2 className="text-3xl sm:text-4xl font-extrabold text-slate-900 tracking-tight">
                {packagesContent.title || "Curated Itineraries & Holiday Packages"}
              </h2>
              <p className="text-xs sm:text-sm text-slate-500 font-medium mt-2">
                {packagesContent.subtitle || "Every tour includes handpicked boutique luxury stays, private scenic transfers, certified local guides, and daily breakfast."}
              </p>
        ...
        {isSectionEnabled(effectiveSections, "pillars") && (
          <section className="py-20 bg-sky-50/50 border-y border-sky-100">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="text-center max-w-xl mx-auto mb-14">
                <span className="text-[11px] font-bold uppercase tracking-[0.25em] text-[#0284C7] block mb-2">
                  {pillarsContent.badge || "Why Travel With Us"}
                </span>
                <h2 className="text-3xl font-extrabold text-slate-900">
                  {pillarsContent.title || "The Luxury Wanderlust Difference"}
                </h2>
                <p className="text-xs text-slate-500 font-medium mt-2">
                  {pillarsContent.subtitle || "We design seamless, transformative journeys backed by 24/7 concierge and verified credentials."}
                </p>
              </div>
              ...
              <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
                <div className="lg:col-span-5 space-y-4">
                  ...
                  <h3 className="text-2xl sm:text-3xl font-black tracking-tight leading-tight">
                    {reviewsContent.title ? `"${reviewsContent.title}"` : '"Our 10-day Switzerland rail journey was effortless from the moment we landed."'}
                  </h3>
                  <p className="text-xs text-slate-300 font-light leading-relaxed">
                    {reviewsContent.subtitle || "Every hotel transfer was punctual, our private guide was deeply knowledgeable, and having a dedicated coordinator gave us complete confidence."}
                  </p>
                  <div className="flex items-center gap-3 pt-2">
                    <div className="w-10 h-10 rounded-full bg-[#0284C7] flex items-center justify-center font-bold text-white text-sm">
                      {reviewsContent.authorInitials || "SK"}
                    </div>
                    <div>
                      <h5 className="font-bold text-xs text-white">{reviewsContent.authorName || "Sameer & Kavita Roy"}</h5>
                      <p className="text-[10px] text-slate-400">{reviewsContent.authorLocation || "Mumbai, India • Swiss Alps Expedition"}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="lg:col-span-7 grid grid-cols-2 gap-4 border-t lg:border-t-0 lg:border-l border-white/10 pt-6 lg:pt-0 lg:pl-8">
                  <div className="bg-white/5 backdrop-blur-xs p-5 rounded-2xl border border-white/10">
                    ...
                    <span className="text-[10px] text-slate-400">Guaranteed scheduled departures</span>
                  </div>
                  <div className="bg-white/5 backdrop-blur-xs p-5 rounded-2xl border border-white/10">
