    ...
    <div className="fixed inset-0 z-[1100] flex items-stretch justify-end" role="dialog" aria-modal="true" aria-label="Shopping cart">
      ...
      <aside className="relative flex h-full w-full max-w-[420px] flex-col bg-cream animate-slide-in-right" ref={panelRef}>
                  ...
                  <div className="flex-1 flex flex-col justify-between">
                    ...
                    <div className="flex items-center justify-between mt-2">
                      <div className="flex items-center border border-cream-dark rounded bg-white">
                        <button 
                          className="h-7 w-7 flex items-center justify-center text-text-muted hover:text-text-dark"
                          onClick={() => onQty(item.productId, item.sku, item.quantity - 1)}
                        >
                      ...
                      <span className="text-sm font-semibold text-text-dark">{formatPrice(product.price * item.quantity)}</span>
                    </div>
                  </div>
                  <button 
                    className="h-8 w-8 flex items-center justify-center text-text-muted hover:text-danger hover:bg-red-50 rounded-full shrink-0"
        ...
        <div className="border-t border-cream-dark p-6 bg-white space-y-4">
          ...
          <button 
            className="w-full min-h-[48px] rounded bg-text-dark text-cream text-xs font-semibold tracking-widest uppercase hover:bg-blue-soft transition-colors duration-200"
